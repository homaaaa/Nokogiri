...
def all_the_prices #extrait le contenu de toutes les balises a.price (prix)
  page = Nokogiri::HTML(open('https://coinmarketcap.com/all/views/all/'))
  page.css('a.price')
end
...
def data_in_array(array1, array2)
  name_array = []
  price_array = []
  for i in (0...array1.size) do #recherche les textes affichés sur le site et les range dans des arrays 
    name_array << array1[i].text
    price_array << array2[i].text
  end
  puts name_array.zip(price_array).map { |name, price| { name => price } } #créé un array de arrays qui contiennent chacun un nom et un prix puis transforme ces arrays en hashs pour faire un array de hashs
  puts ' '
  puts '-' * 20
end
